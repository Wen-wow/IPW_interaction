...
compute.d0<-function(n.d0=1e5,prob.X=0.3,mu.C=0, sd.error=1, sel.prob=0.5, delta1=0.3, delta2=0.3, delta3=0.1, s.model="logit"){
  X<-rbinom(n.d0, 1, prob.X)
  C<-rnorm(n.d0, mu.C, sd.error)
  if (s.model=="logit"){
...
#one simulation
sim<-function(n.sim=1,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob=0.5, delta1=0.3, delta2=0.3, delta3=0.1, n=5,seed=1356,k=1, s.model="logit",y.type="continuous",prob.Y=0.5){
  ...
  delta0<-compute.d0(n.d0=1e5,prob.X=prob.X,mu.C=mu.C, sd.error=sd.error, sel.prob=sel.prob, delta1=delta1, delta2=delta2, delta3=delta3, s.model=s.model)
...
analyse<-function(m=10,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob=0.5, delta1=0.3, delta2=0.3, delta3=0.1, n=1000,seed=1356,k=1,s.model="logit",y.type="continuous", prob.Y=0.5){
  ...
  for (i in 1:m){
    ...
    data.one.simulation<-as.data.frame(sim(n.sim=i,prob.X=prob.X, mu.C=mu.C, alpha.cts = alpha.cts, beta=beta, gamma=gamma,sd.error=sd.error, sel.prob=sel.prob, delta1=delta1, delta2=delta2, delta3=delta3, n=n,seed=seed,k=k, s.model = s.model, y.type = y.type, prob.Y = prob.Y))
...
compare<-function(m=10,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=0.3, delta2=0.3, delta3.range=0.1, n=1e5,seed=1356,k=1,parameter.vary=0.1,s.model="logit",y.type="continuous",prob.Y=0.5){
  ...
  if (all(parameter.vary==sel.prob.range)){ #vary sel.prob.
    for (j in 1:k){
      ...
      data.compare.list[[j]]<-tibble(analyse(m=m,prob.X=prob.X, mu.C=mu.C, alpha.cts=alpha.cts, beta=beta, gamma=gamma,sd.error=sd.error, sel.prob=sel.prob, delta1=delta1, delta2=delta2, delta3=delta3.range, n=n,seed=seed,k=j,s.model = s.model, y.type = y.type, prob.Y = prob.Y))
      print(j)
    }
    
    #For DAG1,the true value is beta=1 if y is continuous; estimate from Y~X if y is binary
    if (y.type=="continuous"){
    ...
    head(per.sum.df)
    write.csv(per.sum.df,paste("v1.0_dag1_",y.type,s.model,"_perform_sel_d3_",delta3.range,".csv"),row.names = F)
    ...
    ggsave(filename =paste("v1.0_dag1_",y.type,s.model,"_sel_d3_",delta3.range,"_bias_with_ci_with0.9.png"),units="in", width=7, height=4, bg = "transparent")
    ...
    ggsave(filename = paste("v1.0_dag1_",y.type,s.model,"_sel_d3_",delta3.range,"_bias_no_ci_with0.9.png"),units="in", width=7, height=4)
    ...
    if (s.model=="logadd"){
      ...
      ggsave(filename =paste("v1.0_dag1_",y.type,s.model,"_sel_d3_",delta3.range,"_bias_with_ci_no0.9.png"),units="in", width=7, height=4, bg = "transparent")
      ...
      ggsave(filename = paste("v1.0_dag1_",y.type,s.model,"_sel_d3_",delta3.range,"_bias_no_ci_no0.9.png"),units="in", width=7, height=4)
    }
    
  }else if (all(parameter.vary==delta3.range)){ #vary delta3.
    for (j in 1:k){
      delta3<-delta3.range[j]
      data.compare.list[[j]]<-tibble(analyse(m=m,prob.X=prob.X, mu.C=mu.C, alpha.cts=alpha.cts, beta=beta, gamma=gamma,sd.error=sd.error, sel.prob=sel.prob.range, delta1=delta1, delta2=delta2, delta3=delta3, n=n,seed=seed,k=j,s.model = s.model, y.type = y.type, prob.Y = prob.Y))
      print(j)
    }
    ...
    if (y.type=="continuous"){
      data.compare<-data.compare.list %>% bind_rows(.id="DGM")  %>% as.data.frame()
      performance<-simsum(data=data.compare,estvarname = "beta.est", true = beta, se="beta.se", methodvar = "method", ref = "True model", by="delta3",x = TRUE)
    }else if (y.type=="binary"){
      data.compare1<-data.compare.list %>% bind_rows(.id="DGM")  %>% as.data.frame()
    ...
    colnames(per.sum.df)<-col.names
    write.csv(per.sum.df, paste("v1.0_dag1_",y.type,s.model,"_perform_delta3_sel_",sel.prob.range,".csv"),row.names = F)
    ...
    ggsave(filename = paste("v1.0_dag1_",y.type,s.model,"_d3_bias_with_ci.png"),units="in", width=7, height=4, bg = "transparent")
    ...
    ggsave(filename = paste("v1.0_dag1_",y.type,s.model,"_d3_bias_no_ci.png"),units="in", width=7, height=4, bg = "transparent")
  }
...
####-----Strue=logit
com.delta3.1<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=0.3, delta2=0.3, delta3.range=seq(-0.3,0.3,length=7), seed=1356,k=1,parameter.vary=seq(-0.3,0.3,length=7),s.model="logit",y.type="continuous", prob.Y=0.5)


#####-----Strue=logadd

com.delta3.2<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=-0.3, delta2=0.1, delta3.range=seq(-0.3,0.3,length=7), seed=1356,k=1,parameter.vary=seq(-0.3,0.3,length=7),s.model="logadd",y.type="continuous",prob.Y=0.5)


####----Strue=probit
com.delta3.3<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=0.3, delta2=0.3, delta3.range=seq(-0.3,0.3,length=7), seed=1356,k=1,parameter.vary=seq(-0.3,0.3,length=7),s.model="probit",y.type="continuous",prob.Y=0.5)
...
com.delta3.4<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=0.3, delta2=0.3, delta3.range=seq(-0.3,0.3,length=7),seed=1356,k=1,parameter.vary=seq(-0.3,0.3,length=7),s.model="logit",y.type="binary", prob.Y=0.5)

#####-----Strue=logadd
com.delta3.5<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=-0.3, delta2=0.1, delta3.range=seq(-0.3,0.3,length=7),seed=1356,k=1,parameter.vary=seq(-0.3,0.3,length=7),s.model="logadd",y.type="binary", prob.Y=0.5)

####----Strue=probit
com.delta3.6<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=0.5, delta1=0.3, delta2=0.3, delta3.range=seq(-0.3,0.3,length=7),seed=1356,k=1,parameter.vary=seq(-0.3,0.3,length=7),s.model="probit",y.type="binary", prob.Y=0.5)
...
####-----Strue=logit
com.delta3.21<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0, seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logit",y.type="continuous", prob.Y=0.5)


#####-----Strue=logadd

com.delta3.22<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=-0.3, delta2=0.1, delta3.range=0, seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logadd",y.type="continuous",prob.Y=0.5)


####----Strue=probit
com.delta3.23<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0, seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="probit",y.type="continuous",prob.Y=0.5)
...
com.delta3.24<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0,seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logit",y.type="binary", prob.Y=0.5)

#####-----Strue=logadd
com.delta3.25<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=-0.3, delta2=0.1, delta3.range=0,seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logadd",y.type="binary", prob.Y=0.5)

####----Strue=probit
com.delta3.26<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0,seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="probit",y.type="binary", prob.Y=0.5)
...
####-----Strue=logit
com.delta3.31<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0.1, seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logit",y.type="continuous", prob.Y=0.5)


#####-----Strue=logadd

com.delta3.32<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=-0.3, delta2=0.1, delta3.range=0.1, seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logadd",y.type="continuous",prob.Y=0.5)


####----Strue=probit
com.delta3.33<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0.1, seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="probit",y.type="continuous",prob.Y=0.5)
...
com.delta3.34<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0.1,seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logit",y.type="binary", prob.Y=0.5)

#####-----Strue=logadd
com.delta3.35<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=-0.3, delta2=0.1, delta3.range=0.1,seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="logadd",y.type="binary", prob.Y=0.5)

####----Strue=probit
com.delta3.36<-compare(m=1000,n=1e5,prob.X=0.3, mu.C=0, alpha.cts=1, beta=1, gamma=1,sd.error=1, sel.prob.range=seq(0.1,0.9,length=5), delta1=0.3, delta2=0.3, delta3.range=0.1,seed=1356,k=1,parameter.vary=seq(0.1,0.9,length=5),s.model="probit",y.type="binary", prob.Y=0.5)
